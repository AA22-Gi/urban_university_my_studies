...
class Iterator:
    ...
    def __init__(self, start, stop, step=1):
        """
        Инициализирует итератор со значениями:
        :param start: Начальное значение итератора
        :param stop: Конечное значение итератора.
        :param step: Значение шага для итератора, по умолчанию 1.
        :param pointer: Указывает на текущее число в итерации (изначально start)
        """
        self.start = start
        self.stop = stop
        self.step = step
    ...
    def __next__(self):
        "Увеличивает атрибут pointer на step и возвращает текущее значение итератора"
        if self.step == 0:
